...
var modal = $(function () {
  ...
  $('.modal-contents').append($('<div>').attr({
    id: 'modal-contents-file-input',
    hidden: 'hidden'
  }).append($('<h1>').text('データソースファイル入力')).append($('<p>').text('入力されたデータソースファイルに基づいてネットワークグラフを作成します。')).append($('<p>').text('下記のデータ作成用テンプレートをコピーまたはダウンロードしてからデータを作成してください。' + '(Googleスプレッドシートを使用してコピーを作成するのが簡単です。)' + 'データ作成方法については、シート「データ作成マニュアル」を参照してください。').append($('<br>')).append($('<a>').attr({
    href: 'https://docs.google.com/spreadsheets/d/1AdNFTfCSotEPVilhd1EvE6poh1fSPZ5TjKZYU9LS318/edit?usp=sharing',
    target: '_blank',
    rel: 'noopener noreferrer'
  }).text('データ作成用テンプレート - relativez_data_template.v01 '))).append($('<p>').text('作成した文献表と引用関係表を選択してください。ファイル形式はカンマ区切り形式(.csv)にしてください。' + 'ファイルや個人情報がサーバーに送信、保存されることはありません。')).append($('<div>').addClass('file-input-box').attr('id', 'nodes-input').append($('<input>').attr({
    type: 'file',
    id: 'nodes-input-file',
    "class": 'file-selector',
    accept: '.csv'
  })).append($('<button>').addClass('file-select-btn').append($('<img>').attr('src', '/assets/Icon-attach-file.svg')).append($('<span>').text('文献表ファイル選択'))).append($('<p>').addClass('file-data')).append($('<p>').addClass('error-msg-box'))).append($('<div>').addClass('file-input-box').attr('id', 'links-input').append($('<input>').attr({
    type: 'file',
    id: 'links-input-file',
    "class": 'file-selector',
    accept: '.csv'
  })).append($('<button>').addClass('file-select-btn').append($('<img>').attr('src', '/assets/Icon-attach-file.svg')).append($('<span>').text('引用関係表ファイル選択'))).append($('<p>').addClass('file-data')).append($('<p>').addClass('error-msg-box'))).append($('<button>').addClass('button-component').attr('id', 'create-graph-btn').text('ネットワークグラフ作成'))); // About

  $('.modal-contents').append($('<div>').attr({
    id: 'modal-contents-about',
    hidden: 'hidden'
  }).append($('<h1>').text('About')).append($('<p>').text('relativezは、文献の引用ネットワークを可視化するWebアプリケーションです。')).append($('<p>').text('目的は下記の3点を達成することです。')).append($('<ul>').append($('<li>').text('文献同士の関係を視覚化することで文脈が把握しやすくなること')).append($('<li>').text('情報量が増えたときに全体をイメージしやすくすること')).append($('<li>').text('研究に必要な文献情報を整理できること'))).append($('<p>').text('relativezが想像する理想的なシーンは、複数の独立したアーティストが集まる場所において、個々人の持つ問題意識について、' + 'お互いがお互いにその複雑さを理解しつつ共通点を見出せる環境です。このWebアプリケーションが会話や勉強会など様々な場面で' + '制作の一助になることを願っています。')).append($('<h1>').text('relativezプロジェクトへの参加方法')).append($('<h2>').text('データ提供者として参加する')).append($('<p>').text('データソースを作成して提供していただくと、relativezにアクセスした他のユーザーが閲覧可能なネットワークグラフとして表示します。' + '「データソース選択」から提供していただいたデータが選択可能になります。')).append($('<p>').text('relativezにデータを提供していただける方はTwitterのDMでご連絡ください。').append($('<br>')).append($('<a>').attr({
    id: 'twitter-link',
    href: 'https://twitter.com/relativez_2021',
    target: '_blank',
    rel: 'noopener noreferrer'
  }).text('Twitter @relativez_2021'))).append($('<h2>').text('開発者として参加する')).append($('<p>').text('このWebアプリはMITライセンスを適用しており、自由に複製・配布・修正されることを無制限に許可したオープンソースプロジェクトです。' + 'ご興味のある方はGitHubでぜひご参加願います。'))); // 免責事項

  $('.modal-contents').append($('<div>').attr({
    id: 'modal-contents-disclaimer',
    hidden: 'hidden'
  }).append($('<h1>').text('免責事項')).append($('<p>').text('本アプリケーションは、ご利用者様に対して何も保証しません。本アプリケーションの関係者(他の利用者も含む)は、' + 'ご利用者に対して一切責任を負いません。ご利用者様が、本アプリケーションを利用(閲覧、参加、外部での再利用など全てを含む)する場合は、自己' + '責任で行う必要があります。')).append($('<ul>').append($('<li>').text('本アプリケーションの利用の結果生じた損害について、一切責任を負いません。')).append($('<li>').text('コンテンツとして提供する全ての文献情報、画像等について、内容の合法性・正確性・安全性等、あらゆる点にお' + 'いて保証しません。本アプリケーションはこれらの情報をご利用者様が利用することによって生じたいかなる損害に対しても一切責任を負いません。')).append($('<li>').text('利用者同士のトラブルには、一切介入致しません。')).append($('<li>').text('ご利用者様の適用される法令に照らして、本アプリケーションの利用が合法であることを保証しません。')).append($('<li>').text('リンクをしている外部サイトについては、何ら保証しません。')).append($('<li>').text('本アプリケーションは、ご利用者様が本アプリケーションそのものおよびそのコンテンツを外部で2次利用した場合、' + 'MITライセンスの定められた要件を充たす限りご利用者様に対して著作権侵害を主張することはありません。しかし、本アプリケーション以外の第三者から' + '著作権侵害を理由に訴えられないことを保証するものではありません。')).append($('<li>').text('利用者の提供したデータソースは、本アプリケーション関係者の基準に従って掲載の可否を判断し、その掲載を断る可能性' + 'がありますが、そのことによって生じた損害について一切責任を負いません。')).append($('<li>').text('事前の予告無く、アプリケーションの提供を中止する可能性があります。')))); // データソース選択

  $('.modal-contents').append($('<div>').attr({
    id: 'modal-contents-file-select',
    hidden: 'hidden'
  }).append($('<h1>').text('データソース選択')).append($('<p>').text('選択されたデータソースに基づくネットワークグラフを表示します。relativezは以下のデータソースを提供しています。')).append($('<label>').attr('for', 'data-selection').text('データソースを選択してくだい。')).append($('<div>').attr('id', 'select-box-wrapper').append($('<select>').attr({
    id: 'data-selection',
    name: 'data-selection'
...
window.$ = window.jQuery = require('jquery'); // eslint-disable-next-line no-unused-vars

var modal = require('./modal.js'); // eslint-disable-next-line no-unused-vars


var responsive = require('./responsive.js');

var metaDataSelection;
d3.json('data/data_selection.json').then(function (data) {
...
createArrow(svg);
Promise.all([d3.blob('data/initial_graph_nodes.csv'), d3.blob('data/initial_graph_links.csv')]).then(function (blob) {
  var nodeObjUrl = URL.createObjectURL(blob[0]);
  var linksObjUrl = URL.createObjectURL(blob[1]);
...
$('input[name="highlight-citation-link"]').on('change', function () {
  ...
  if (propCitationCheckBox) {
    d3.selectAll('[data-linkType="citation"]').style('stroke', COLOR.CITATION).attr('marker-end', 'url(#arrow-citation)');
  } else {
...
$('input[name="hightlight-cited-by-link"]').on('change', function () {
  ...
  if (propCitedByChechBox) {
    d3.selectAll('[data-linkType="cited-by"]').style('stroke', COLOR.CITEDBY).attr('marker-end', 'url(#arrow-cited-by)');
  } else {
...
function showSelectedGraph() {
  ...
  Promise.all([d3.blob(metaDataSelection[keyName].nodes_path), d3.blob(metaDataSelection[keyName].links_path)]).then(function (blob) {
    var nodeObjUrl = URL.createObjectURL(blob[0]);
    var linksObjUrl = URL.createObjectURL(blob[1]);
    createGraph(nodeObjUrl, linksObjUrl);
...
function onChangeFileInput() {
  ...
  if (file.name.endsWith('.csv')) {
    $('.modal').find('#' + inputId).children('.file-data').append($('<div></div>').text("\u30D5\u30A1\u30A4\u30EB\u540D : ".concat(file.name))).append($('<div></div>').text("\u30D5\u30A1\u30A4\u30EB\u30B5\u30A4\u30BA : ".concat(returnFileSize(file.size))));
  } else {
    var errorMsg = '入力不可能なファイル形式です。カンマ区切り形式のファイル(.csv)を選択してください。';
...
function onClickCreateGraph() {
  ...
  if (nodesFileInput.files.length === 0) {
    var errorMsg = '文献表ファイルを選択してください。';
    $('#nodes-input .error-msg-box').text(errorMsg);
    return;
  } else if (linksFileInput.files.length === 0) {
    var _errorMsg = '引用関係表ファイルを選択してください。';
    $('#links-input .error-msg-box').text(_errorMsg);
    return;
  }

  if (nodesFile.size >= 1048576) {
    var _errorMsg2 = 'ファイルサイズは1MB未満にしてください。';
    $('#nodes-input .error-msg-box').text(_errorMsg2);
    return;
  } else if (linksFile.size >= 1048576) {
    var _errorMsg3 = 'ファイルサイズは1MB未満にしてください。';
    $('#links-input .error-msg-box').text(_errorMsg3);
    return;
  }

  var nodesPromise = nodesFile.text();
  var linksPromise = linksFile.text();
  Promise.all([nodesPromise, linksPromise]).then(function (fileText) {
    var nodesText = fileText[0];
    var linksText = fileText[1];
    ...
    if (!nodesText.startsWith('node_id,title,author,year,translator,original_work,publisher,publication_detail,link_text_1,link_url_1,link_text_2,link_url_2,link_text_3,link_url_3,others')) {
      var _errorMsg4 = '文献表ファイルのCSVデータ構造に問題があります。正しいデータ構造の文献表ファイルを入力してください。';
      $('#nodes-input .error-msg-box').text(_errorMsg4);
      throw new Error('csv data format error.');
    } else if (!linksText.startsWith('source,source_title,target,target_title')) {
      var _errorMsg5 = '引用関係表ファイルのCSVデータ構造に問題があります。正しいデータ構造の引用関係表ファイルを入力してください。';
      $('#links-input .error-msg-box').text(_errorMsg5);
      throw new Error('csv data format error.');
    }
...
function createGraph(nodesObjUrl, linksObjUrl) {
  Promise.all([d3.csv(nodesObjUrl), d3.csv(linksObjUrl)]).then(function (data) {
    URL.revokeObjectURL(nodesObjUrl);
    URL.revokeObjectURL(linksObjUrl);
    var nodes = data[0];
    var links = data[1];
    validateData(nodes, links);
    nodes = addCharToYear(nodes); // グラフのスケールと表示位置の調整用
    ...
    link.exit().remove();
    var linkEnter = link.enter().append('path').attr('class', 'link').attr('stroke-width', 2).style('stroke', COLOR.LINK_INIT).attr('stroke-opacity', 0.7).attr('fill', 'none').attr('marker-end', 'url(#arrow)');
    link = linkEnter.merge(link); // move update selection nodes to the front
    ...
    var tip = d3.select('body').append('div').attr('class', 'tooltip').style('opacity', 0); // 隣接ノードか調べるためのmap
    ...
    function mouseover(event, d) {
      tip.style('opacity', 1).text(d.title).style('left', event.pageX + 6 + 'px').style('top', event.pageY + 6 + 'px');
    }
    ...
    function onClickNode(event, d) {
      ...
      node.select('circle').attr('stroke', function (n) {
        if (neighboring(d, n)) {
          return COLOR.WHITE;
        }
    ...
    function showCurrentText(d) {
      var textListItem = createListItems(d);
    ...
    function showCitationText(d) {
      nodes.forEach(function (n) {
        if (neighboringCitation(d, n)) {
          var textListItem = createListItems(n);
...
function validateData(nodes, links) {
  var nodesErrorMessages = [];
  var linksErrorMessages = [];
  var rules = {
    node_id: {
      regexp: /[0-9]/,
  ...
  if (nodes.length > 550) {
    nodesErrorMessages.push("\u6587\u732E\u6570\u304C\u591A\u3059\u304E\u307E\u3059\u3002\u73FE\u5728\u306E\u6587\u732E\u6570\u306F ".concat(nodes.length, " \u3067\u3059\u3002\u6587\u732E\u6570\u306F550\u4EF6\u4EE5\u4E0B\u306B\u3057\u3066\u304F\u3060\u3055\u3044\u3002'\n      "));
  }

  for (var i = 0; i < nodes.length; i++) {
    if (!rules.node_id.regexp.test(nodes[i].node_id)) {
      nodesErrorMessages.push("".concat(i + 1, " \u884C\u76EE\u306E\u30C7\u30FC\u30BF : ").concat(rules.node_id.error));
    }

    if (!rules.title.regexp.test(nodes[i].title)) {
      nodesErrorMessages.push("".concat(i + 1, " \u884C\u76EE\u306E\u30C7\u30FC\u30BF : ").concat(rules.title.error));
    }

    if (!rules.author.regexp.test(nodes[i].author)) {
      nodesErrorMessages.push("".concat(i + 1, " \u884C\u76EE\u306E\u30C7\u30FC\u30BF : ").concat(rules.author.error));
    }

    if (!rules.year.regexp.test(nodes[i].year)) {
      nodesErrorMessages.push("".concat(i + 1, " \u884C\u76EE\u306E\u30C7\u30FC\u30BF : ").concat(rules.year.error));
    }

    if (!rules.translator.regexp.test(nodes[i].translator)) {
      nodesErrorMessages.push("".concat(i + 1, " \u884C\u76EE\u306E\u30C7\u30FC\u30BF : ").concat(rules.translator.error));
    }

    if (!rules.original_work.regexp.test(nodes[i].original_work)) {
      nodesErrorMessages.push("".concat(i + 1, " \u884C\u76EE\u306E\u30C7\u30FC\u30BF : ").concat(rules.original_work.error));
    }

    if (!rules.publisher.regexp.test(nodes[i].publisher)) {
      nodesErrorMessages.push("".concat(i + 1, " \u884C\u76EE\u306E\u30C7\u30FC\u30BF : ").concat(rules.publisher.error));
    }

    if (!rules.publication_detail.regexp.test(nodes[i].publication_detail)) {
      nodesErrorMessages.push("".concat(i + 1, " \u884C\u76EE\u306E\u30C7\u30FC\u30BF : ").concat(rules.publication_detail.error));
    }

    if (!rules.link_text_1.regexp.test(nodes[i].link_text_1)) {
      nodesErrorMessages.push("".concat(i + 1, " \u884C\u76EE\u306E\u30C7\u30FC\u30BF : ").concat(rules.link_text_1.error));
    }

    if (!rules.link_text_2.regexp.test(nodes[i].link_text_2)) {
      nodesErrorMessages.push("".concat(i + 1, " \u884C\u76EE\u306E\u30C7\u30FC\u30BF : ").concat(rules.link_text_2.error));
    }

    if (!rules.link_text_3.regexp.test(nodes[i].link_text_3)) {
      nodesErrorMessages.push("".concat(i + 1, " \u884C\u76EE\u306E\u30C7\u30FC\u30BF : ").concat(rules.link_text_3.error));
    }

    if (!rules.others.regexp.test(nodes[i].others)) {
      nodesErrorMessages.push("".concat(i + 1, " \u884C\u76EE\u306E\u30C7\u30FC\u30BF : ").concat(rules.others.error));
    }
  }

  for (var _i = 0; _i < links.length; _i++) {
    if (!rules.source.regexp.test(links[_i].source)) {
      linksErrorMessages.push("".concat(_i + 1, " \u884C\u76EE\u306E\u30C7\u30FC\u30BF\uFF1A' ").concat(rules.source.error));
    }

    if (!rules.target.regexp.test(links[_i].target)) {
      linksErrorMessages.push("".concat(_i + 1, " \u884C\u76EE\u306E\u30C7\u30FC\u30BF\uFF1A' ").concat(rules.target.error));
    }
  }
...
function addCharToYear(nodes) {
  ...
  var _loop = function _loop(_i2) {
    var nodeId = nodes[_i2].node_id;
    var currentAuthor = nodes[_i2].author;
    var currentYear = nodes[_i2].year;
    sameAuthorYearNodes = [];

    for (var j = 0; j < sameAuthorYearNodesSets.length; j++) {
  ...
  var _loop2 = function _loop2(_i3) {
    var _loop3 = function _loop3(j) {
      var nodeAddChar = nodes.find(function (node) {
        return node.node_id === sameAuthorYearNodesSets[_i3][j];
      });
      nodeAddChar.year_added_char = nodeAddChar.year + ' ' + ALPHABETCHAR[j];
    };

    for (var j = 0; j < sameAuthorYearNodesSets[_i3].length && j < 26; j++) {
      _loop3(j);
    }
  };

  for (var _i3 = 0; _i3 < sameAuthorYearNodesSets.length; _i3++) {
    _loop2(_i3);
  }
...
function handleZoom(e) {
  ...
  d3.select('svg g').attr('transform', "".concat(e.transform, " translate(").concat(clientWidth * adjustLayout, ", ").concat(clientHeight * adjustLayout, ") scale(").concat(graphScale, ")"));
}

function initZoom() {
  svg.attr('transform', "translate(".concat(clientWidth * adjustLayout, ", ").concat(clientHeight * adjustLayout, ") scale(").concat(graphScale, ")"));
  d3.select('svg').call(zoom);
}

function createArrow(svg) {
  // arrow-head デフォルト
  svg.append('defs').append('marker').attr('id', 'arrow').attr('refX', 16).attr('refY', 5.8).attr('markerWidth', 28).attr('markerHeight', 28).attr('orient', 'auto').append('path').attr('d', 'M2,2 L10,6 L2, 10 L6,6 L2,2').style('fill', COLOR.LINK_INIT); // arrow-head 引用線用

  d3.select('defs').append('marker').attr('id', 'arrow-citation').attr('refX', 16).attr('refY', 5.8).attr('markerWidth', 28).attr('markerHeight', 28).attr('orient', 'auto').append('path').attr('d', 'M2,2 L10,6 L2, 10 L6,6 L2,2').style('fill', COLOR.CITATION); // arrow-head 被引用線用

  d3.select('defs').append('marker').attr('id', 'arrow-cited-by').attr('refX', 16).attr('refY', 5.8).attr('markerWidth', 28).attr('markerHeight', 28).attr('orient', 'auto').append('path').attr('d', 'M2,2 L10,6 L2, 10 L6,6 L2,2').style('fill', COLOR.CITEDBY);
}
...
function createListItems(node) {
  ...
  if (node.translator) {
    textListItem.append($('<div></div>').text("\u7FFB\u8A33\u8005 : ".concat(node.translator)));
  }

  if (node.publication_detail) {
    textListItem.append($('<div></div>').text("\u63B2\u8F09\u5143 : ".concat(node.publication_detail)));
  }

  if (node.publisher) {
    textListItem.append($('<div></div>').text("\u767A\u884C\u6240 : ".concat(node.publisher)));
  }
  ...
  if (node.link_url_1 || node.link_url_2 || node.link_url_3) {
    ...
    if (node.link_url_1.match(/^https?:\/\//)) {
      if (node.link_text_1 && node.link_url_1) {
        textListItem.find('ul').append($('<li></li>').append($('<a></a>').attr({
  ...
  if (node.others) {
    textListItem.append($('<div></div>').text("\u305D\u306E\u4ED6 : ".concat(node.others)));
  }
...
},{"./modal.js":1,"./responsive.js":2,"jquery":4}],4:[function(require,module,exports){
/*!
