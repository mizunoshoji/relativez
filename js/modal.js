...
const modal = $(function () {
  ...
  $('.modal-contents').append(
    $('<div>')
      ...
      .append(
        $('<p>').text(
          'relativezが想像する理想的なシーンは、複数の独立したアーティストが集まる場所において、個々人の持つ問題意識について、' +
  ...
  $('.modal-contents').append(
    $('<div>')
      ...
      .append(
        $('<p>').text(
          '本アプリケーションは、ご利用者様に対して何も保証しません。本アプリケーションの関係者(他のご利用者様も含む)は、' +
            'ご利用者様に対して一切責任を負いません。ご利用者様が、本アプリケーションを利用(閲覧、参加、外部での再利用など全てを含む)する場合は、自己' +
            '責任で行う必要があります。'
        )
      )
      .append($('<p>').text('本アプリケーションの利用の結果生じた損害について、一切責任を負いません。'))
      .append(
        $('<p>').text(
      ...
      .append($('<p>').text('ご利用者様同士のトラブルには一切介入致しません。'))
      .append(
        $('<p>').text('ご利用者様の適用される法令に照らして、本アプリケーションの利用が合法であることを保証しません。')
      )
      .append($('<p>').text('リンクをしている外部サイトについては何ら保証しません。'))
      .append(
        $('<p>').text(
      ...
      .append(
        $('<p>').text(
          'ご利用者様の提供したデータソースは、本アプリケーション関係者の基準に従って掲載の可否を判断し、その掲載を断る可能性' +
            'がありますが、そのことによって生じた損害について一切責任を負いません。'
        )
      )
      .append($('<p>').text('事前の予告無く、アプリケーションの提供を中止する可能性があります。'))
  )
