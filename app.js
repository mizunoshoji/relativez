...
var modal = $(function () {
  ...
  $('.modal-contents').append($('<div>').attr({
    ...
    id: 'data-selection',
    name: 'data-selection'
  }).append($('<option>').attr('value', 'initial_graph').text('日本美術の制度論系 (作成者：大原由)')).append($('<option>').attr('value', 'sample_1').text('サンプル1')).append($('<option>').attr('value', 'sample_2').text('サンプル2')))).append($('<button>').addClass('button-component').attr('id', 'show-selected-graph-btn').text('ネットワークグラフ表示')));
...
$(function () {
  var graphScale;
  var adjustLayout;
  var keepedGraphState = '';
  var clientWidth = document.documentElement.clientWidth;
  var clientHeight = document.documentElement.clientHeight; // SVG graph group

  var svg = d3.select('#vis').append('svg').append('g'); // arrow-head デフォルト

  svg.append('defs').append('marker').attr('id', 'arrow').attr('refX', 16).attr('refY', 5.8).attr('markerWidth', 28).attr('markerHeight', 28).attr('orient', 'auto').append('path').attr('d', 'M2,2 L10,6 L2, 10 L6,6 L2,2').style('fill', '#444'); // arrow-head 引用線用

  d3.select('defs').append('marker').attr('id', 'arrow-citation').attr('refX', 16).attr('refY', 5.8).attr('markerWidth', 28).attr('markerHeight', 28).attr('orient', 'auto').append('path').attr('d', 'M2,2 L10,6 L2, 10 L6,6 L2,2').style('fill', '#c44caa'); // arrow-head 被引用線用
  ...
  function validateData(nodes, links) {
    ...
    for (var _i = 0; _i < links.length; _i++) {
      if (!rules.source.regexp.test(links[_i].source)) {
        linksErrorMessages.push(_i + 1 + '行目のデータ：' + rules.source.error);
      }

      if (!rules.target.regexp.test(links[_i].target)) {
        linksErrorMessages.push(_i + 1 + '行目のデータ：' + rules.target.error);
      }
    }
  ...
  function createGraph(nodesObjUrl, linksObjUrl) {
    Promise.all([d3.csv(nodesObjUrl), d3.csv(linksObjUrl)]).then(function (data) {
      ...
      nodes = addCharToYear(nodes); // scale change
      ...
      var link = svg.selectAll('.link').data(links); // SVG node

      var node = svg.selectAll('.node').data(nodes); // simulation

      var simulation = d3.forceSimulation(nodes).force('link', d3.forceLink().links(links).id(function (n) {
        return n.node_id;
      }).distance(200)).force('charge', d3.forceManyBody().strength(-30)).force('center', d3.forceCenter(clientWidth * 0.5, clientHeight * 0.5)).force('collige', d3.forceCollide().radius(60).strength(1).iterations(4)).velocityDecay(0.4).alphaMin(0.15).on('tick', ticked); // tick
      ...
      link.exit().remove();
      var linkEnter = link.enter().append('path').attr('class', 'link').attr('stroke-width', 2).style('stroke', '#444').attr('stroke-opacity', 0.7).attr('fill', 'none').attr('marker-end', 'url(#arrow)');
      link = linkEnter.merge(link); // move update selection nodes to the front

      $('.node').appendTo('#vis > svg > g');
      node.exit().remove();
      var nodeEnter = node.enter().append('g').attr('class', 'node');
      nodeEnter.append('circle').attr('r', '10').attr('fill', '#666').style('z-index', '2');
      nodeEnter.append('text').attr('dx', -30).attr('dy', -16).attr('font-size', 12).attr('fill', '#eaeaea').text(function (d) {
      ...
      node.call(d3.drag().on('start', dragstarted).on('drag', dragged).on('end', dragended)); // define tooltip
      ...
      function nodeClick(event, d) {
        ...
        var propCitedByChechBox = $('#hightlight-cited-by-link').prop('checked'); // 関連リンクをハイライト
        ...
        d3.select(this).select('circle').attr('fill', '#4169e1').attr('stroke-width', 4).attr('stroke', '#eaeaea'); // 選択中ノードにid付与
      ...
      function showCurrentText(d) {
        var textListItem = textListItemTemplete(d);
      ...
      function showCitationText(d) {
        nodes.forEach(function (n) {
          if (neighboringCitation(d, n)) {
            var textListItem = textListItemTemplete(n);
      ...
      function textListItemTemplete(node) {
        var textListItem = $('<li></li>').append($('<div></div>').addClass('text-title').text(node.title)).append($('<div></div>').text('著者 : ' + node.author)).append($('<div></div>').text('発行年 : ' + node.year));
  ...
  $('input[name="highlight-citation-link"]').on('change', function () {
    ...
    if (propCitationCheckBox) {
      d3.selectAll('[data-linkType="citation"]').style('stroke', '#c44caa').attr('marker-end', 'url(#arrow-citation)');
    } else {
      d3.selectAll('[data-linkType="citation"]').style('stroke', '#444').attr('marker-end', 'url(#arrow)');
    }
  }); // サイドバー開閉
  ...
  $('input[name="hightlight-cited-by-link"]').on('change', function () {
    ...
    if (propCitedByChechBox) {
      d3.selectAll('[data-linkType="cited-by"]').style('stroke', '#bfa925').attr('marker-end', 'url(#arrow-cited-by)');
    } else {
      d3.selectAll('[data-linkType="cited-by"]').style('stroke', '#444').attr('marker-end', 'url(#arrow)');
    }
  });
  ...
  $('#create-graph-btn').on('click', onClickCreateGraph); // データソース選択
  ...
  function onChangeFileInput() {
    ...
    if (file.name.endsWith('.csv')) {
      $('.modal').find('#' + inputId).children('.file-data').append($('<div></div>').text('ファイル名 : ' + file.name)).append($('<div></div>').text('ファイルサイズ : ' + returnFileSize(file.size)));
    } else {
  ...
  function showSelectedGraph() {
    ...
    Promise.all([d3.blob(dataSelection[keyName].nodes_path), d3.blob(dataSelection[keyName].links_path)]).then(function (blob) {
      var nodeObjUrl = URL.createObjectURL(blob[0]);
      var linksObjUrl = URL.createObjectURL(blob[1]);
      createGraph(nodeObjUrl, linksObjUrl);
