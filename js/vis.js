  ...
  function initZoom() {
    if (keepedGraphState === '') {
      svg.attr(
        'transform',
        'translate(' +
          clientWidth * adjustLayout +
          ', ' +
          clientHeight * adjustLayout +
          ') scale(' +
          graphScale +
          ')'
      )
  ...
  function validateData(nodes, links) {
    ...
    const rules = {
      ...
      link_text_1: {
        regexp: /^.{0,140}$/,
        error:
          'link_text_1(オンラインリンク文字列1)は140文字以内で入力してください。'
      },
      link_text_2: {
        regexp: /^.{0,140}$/,
        error:
          'link_text_2(オンラインリンク文字列2)は140文字以内で入力してください。'
      },
      link_text_3: {
        regexp: /^.{0,140}$/,
        error:
          'link_text_3(オンラインリンク文字列3)は140文字以内で入力してください。'
      },
      others: {
        regexp: /^.{0,140}$/,
    ...
    if (nodes.length > 550) {
      nodesErrorMessages.push(
        '文献数が多すぎます。現在の文献数は' +
          nodes.length +
          'です。文献数は550件以下にしてください。'
      )
    }
  ...
  function addCharToYear(nodes) {
    ...
    for (let i = 0; i < sameAuthorYearNodesSets.length; i++) {
      for (let j = 0; j < sameAuthorYearNodesSets[i].length && j < 26; j++) {
        const nodeAddChar = nodes.find(
          (node) => node.node_id === sameAuthorYearNodesSets[i][j]
        )
    ...
    Promise.all([d3.csv(nodesObjUrl), d3.csv(linksObjUrl)])
      .then(function (data) {
        ...
        const simulation = d3
          ...
          .force(
            'center',
            d3.forceCenter(clientWidth * 0.5, clientHeight * 0.5)
          )
          .force(
            'collige',
            d3.forceCollide().radius(60).strength(1).iterations(4)
          )
          .velocityDecay(0.4)
          .alphaMin(0.15)
          .on('tick', ticked)
        ...
        function dragstarted(event, d) {
          if (!event.active)
            simulation.alphaTarget(0.3).velocityDecay(1).restart()
          d.fx = d.x
          d.fy = d.y
        }
        ...
        function nodeClick(event, d) {
          ...
          const propCitationCheckBox = $('#highlight-citation-link').prop(
            'checked'
          )
          const propCitedByChechBox = $('#hightlight-cited-by-link').prop(
            'checked'
          )
        ...
        function showCurrentText(d) {
          const textListItem = textListItemTemplete(d)
        ...
        function showCitationText(d) {
          nodes.forEach(function (n) {
            if (neighboringCitation(d, n)) {
              const textListItem = textListItemTemplete(n)
        ...
        function textListItemTemplete(node) {
          ...
          if (node.original_work) {
            textListItem.append(
              $('<div></div>').text('原著 : ' + node.original_work)
            )
          }
          if (node.translator) {
            textListItem.append(
              $('<div></div>').text('翻訳者 : ' + node.translator)
            )
          }
          if (node.publication_detail) {
            textListItem.append(
              $('<div></div>').text('掲載元 : ' + node.publication_detail)
            )
          }
          if (node.publisher) {
            textListItem.append(
              $('<div></div>').text('発行所 : ' + node.publisher)
            )
          }
          if (node.link_url_1 || node.link_url_2 || node.link_url_3) {
            textListItem.append(
              $('<div>オンラインリンク : </div>').append($('<ul></ul>'))
            )
  ...
  function onChangeFileInput() {
    ...
    if (file.name.endsWith('.csv')) {
      $('.modal')
        ...
        .append(
          $('<div></div>').text('ファイルサイズ : ' + returnFileSize(file.size))
        )
    } else {
      const errorMsg =
        '入力不可能なファイル形式です。カンマ区切り形式のファイル(.csv)を選択してください。'
      $('.modal')
        .find('#' + inputId)
        .children('.error-msg-box')
        .text(errorMsg)
    ...
    Promise.all([nodesPromise, linksPromise])
      .then(function (fileText) {
        ...
        if (
          ...
          !linksText.startsWith('source,source_title,target,target_title')
        ) {
          const errorMsg =
